...
from tfops.loss import sigmoid_cross_entropy_without_mean, mse_10x, vae_kl_cost_weight
...
class Model(ModelPlugin):
    ...
    def build(self):
        ...
        self.n_gene = 10000
        self.input1 = tf.placeholder(tf.float32, shape = [self.args.nbatch, self.n_gene])
        ...
        dec_dict = self.decoder_net(z=tf.concat([self.z_sample, self.objective], axis=-1), output_channel=self.n_gene, scope="decoder", reuse=False)
        self.dec_output = dec_dict['output']
        self.lowD_x = dec_dict['h']

        # Unary vector
        self.rec_cost_vector = sigmoid_cross_entropy_without_mean(labels=self.input1, logits=self.dec_output)
        # self.rec_cost_vector = mse_10x(self.input1, self.dec_output)
        # Loss
        ...
        dec_dict = self.decoder_net(z=self.latent_ph, output_channel=self.n_gene, scope="decoder", reuse=True)
        self.dec_output_ph = dec_dict['output']
        self.lowD_x_ph = dec_dict['h']
    ...
    def evaluate(self, print_option=False, include_discrete=False, eps=1e-8, nsample=1024):
        ...
        data_file_id = '/home/yeganeh/github/cascadeVAE/10x/Dsprites_exp/model/data_file_' + current_time
        total_mean, total_std, latent_cat, lowD_x = self.get_latent_total()
    ...
    def get_lowd_space(self):
        return apply_tf_op_multi_input(inputs_list=[self.image, np.zeros([self.ndata, self.args.nconti]), np.zeros([self.ndata, self.args.ncat])], session=self.sess, input_gate_list=[self.input1, self.epsilon_input, self.objective], output_gate=self.lowD_x, batch_size=self.args.nbatch, train_gate=self.istrain)
    def get_latent_total(self):
        total_mean, total_std = self.get_mean_std()
        lowD_x = self.get_lowd_space()
        ...
        return total_mean, total_std, latent_cat, lowD_x
