...
class Trainer():
    ...
    def train(self, data_loader, epochs=10, save_training_gif=None):
        ...
        for epoch in range(epochs):
            ...
            print('Epoch: {}, Average loss: {:.2f}, time: {:.2f}'.format(epoch
                                                                       + 1,
    ...
    def _train_epoch(self, data_loader):
        ...
        ti = np.zeros(len(data_loader))
        for batch_idx, (data, label) in enumerate(data_loader):
            t0 = time.time()
            iter_loss = self._train_iteration(data)
            ti[batch_idx] = time.time() - t0
    ...
    def _train_iteration(self, data):
        ...
        recon_batch, _, latent_dist = self.model(data)
        loss, rec_loss = self._loss_function(data, recon_batch, latent_dist)
        loss.backward()
        self.optimizer.step()
