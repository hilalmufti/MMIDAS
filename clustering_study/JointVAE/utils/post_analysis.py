...
from utils.celltype_hierarchy import hierarchy_plot, \
    cell_nodes_dict, \
    heatmap_plot
...
def cell_category_hist(cluster_per_cat,
    ...
    if cpl_flag:
        for arm in range(n_arm):
            ...
            fig.colorbar(im, ax=ax)
            im.set_clim(0, 2)
            ax.set_ylabel('Cell Types', fontsize=36)
            ax.set_xlabel('Cell Types', fontsize=36)
...
def get_merged_types(htree_file, cells_labels, num_classes=0, node='n4'):
    ...
    # kept_subtree.plot((6, 3), fontsize=12, txtleafonly=True,
    #                   skeletoncol='#444444', fig=fig_tree)
    # plt.show()
    ...
    #mod_subtree.plot((6, 3), fontsize=12, txtleafonly=True,
    #                 skeletoncol='#444444', fig=None)
    #plt.show()

    # print('\n\n---- Plot: Leaf nodes evenly distributed: ----')
    minx = np.min(mod_subtree.x)
    maxx = np.max(mod_subtree.x)

    mod_subtree.update_layout()
    # newminx = np.min(mod_subtree.x)
    # newmaxx = np.max(mod_subtree.x)
    # mod_subtree.x = (mod_subtree.x)
        #             *(maxx-minx)/(newmaxx-newminx)*(
        # np.unique(merged_cells_labels).size/
        # np.unique(cells_labels).size)
