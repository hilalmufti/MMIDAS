...
def load_data(datafile, n_gene=0, gene_id=[], rmv_type=[], min_num=10, eps=1e-1, tau=1.0):
    ...
    for ttype in rmv_type:
        ind = np.where(data['cluster'] == ttype)[0]
        for key in data:
            if key not in ['gene_id']:
                data[key] = np.delete(data[key], ind, axis=0)

    uniq_clusters = np.unique(data['cluster'])
    count = np.zeros(len(uniq_clusters))
    for it, tt in enumerate(uniq_clusters):
        count[it] = sum(data['cluster'] == tt)

    uniq_clusters = uniq_clusters[count >= min_num]
    data['cluster_id'] = np.zeros(len(data['cluster']))
    for ic, cls_ord in enumerate(np.unique(data['cluster'])):
        data['cluster_id'][data['cluster'] == cls_ord] = int(ic + 1)
    ...
    data['n_type'] = len(np.unique(data['cluster']))

    print(' --------- Data Summary --------- ')
    print(f'num cell types: {len(np.unique(data["cluster"]))}, num cells: {data["log1p"].shape[0]}, num genes:{len(data["gene_id"])}')
    
    return data


def data_gen(dataset, train_size, index):

        test_size = dataset.shape[0] - train_size
        train_cpm, test_cpm, train_ind, test_ind = train_test_split(
            dataset[:, index], np.arange(dataset.shape[0]), train_size=train_size, test_size=test_size, random_state=0)
...
def get_loaders(dataset, label=[], index=[], batch_size=128, train_size=0.9):

        batch_size = batch_size

        if len(index) == 0:
            index = np.arange(0, dataset.shape[1])
        ...
        if len(label) > 0:
            ...
            for ll in np.unique(label):
                ...
                _, _, _, train_subind, val_subind, test_subind = data_gen(dataset[indx, :], tt_size, index)
            ...
            train_set, val_set, test_set, train_ind, val_ind, test_ind = data_gen(dataset, tt_size, index)
        ...
        data_set_troch = torch.FloatTensor(dataset[:, index])
