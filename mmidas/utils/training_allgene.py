...
class train_cplmixVAE:
    ...
    def data_gen(self, dataset, train_size):
        ...
        train_cpm, test_cpm, train_ind, test_ind = train_test_split(
            dataset[:, self.index], np.arange(dataset.shape[0]), train_size=train_size, test_size=test_size)
    ...
    def getdata(self, dataset, label=[], index=[], batch_size=128, train_size=0.9):
        ...
        kwargs = {'num_workers': 4, 'pin_memory': True, 'shuffle': True, 'batch_size': batch_size}
    ...
    def init_model(self, n_categories, state_dim, input_dim, fc_dim=100, lowD_dim=5, x_drop=0.2, s_drop=0.2, lr=.001,
                   lam=1, lam_pc=1, n_arm=2, temp=1., tau=0.01, beta=1., hard=False, state_det=False, ref_prior=False,
        ...
        self.model = cpl_mixVAE(input_dim=self.input_dim, fc_dim=fc_dim, n_categories=self.n_categories, state_dim=self.state_dim,
                                ...
                                tau=tau, beta=beta, hard=hard, state_det=state_det, device=self.device, eps=self.eps,
    ...
    def run(self, train_loader, test_loader, validation_loader, alldata_loader, n_epoch, n_epoch_p, c_onehot=0, c_p=0, min_con=.5, max_pron_it=0):
        """
        ...
        e_p = np.linspace(1, n_epoch, n_epoch)
        temp = np.exp(np.log(self.temp) * e_p / n_epoch)
        for epoch in range(n_epoch):
            ...
            for batch_indx, (data, d_idx), in enumerate(train_loader):
                ...
                if self.ref_prior:
                    c_bin = torch.FloatTensor(c_onehot[d_idx, :])
                    prior_c = torch.FloatTensor(c_p[d_idx, :])
                    if self.gpu:
                        c_bin = c_bin.cuda(self.device)
                ...
                recon_batch, x_low, qc, s, c, mu, log_var, log_qc = self.model(x=trans_data, temp=temp[epoch], prior_c=prior_c)
                loss, loss_rec, loss_joint, entropy, dist_c, d_qc, KLD_cont, min_var_0, loglikelihood = self.model.loss(recon_batch, trans_data, mu, log_var, qc, c, c_bin)
                loss.backward()
                self.optimizer.step()
            ...
            with torch.no_grad():
                ...
                for batch_indx, (data_val, d_idx), in enumerate(validation_loader):
                    ...
                    for arm in range(self.n_arm):
                       g_ind0 = arm * self.input_dim
                       g_ind1 = (arm + 1) * self.input_dim
                       trans_val_data.append(data_val[:, g_ind0:g_ind1])
                    ...
                    recon_batch, x_low, qc, s, c, mu, log_var, _ = self.model(x=trans_val_data, temp=temp[epoch], prior_c=prior_c, eval=True)
                    loss, loss_rec, loss_joint, _, _, _, _, _, _ = self.model.loss(recon_batch, trans_val_data, mu, log_var, qc, c, c_bin)
        ...
        while not stop_prune:
            ...
            with torch.no_grad():
                for i, (data, d_idx) in enumerate(alldata_loader):
                    ...
                    recon, x_low, z_category, state, z_smp, mu, log_sigma, _ = self.model(trans_data, self.temp, prior_c=prior_c, mask=pruning_mask, eval=True)
                    loss, loss_arms, loss_joint, _, _, _, _, _, _ = self.model.loss(recon, trans_data, mu, log_sigma, z_category, z_smp, c_bin)
            ...
            if not stop_prune:
                ...
                e_p = np.linspace(1, n_epoch_p, n_epoch_p)
                temp = np.exp(np.log(self.temp) * e_p / n_epoch_p)
                for epoch in range(n_epoch_p):
                    ...
                    for batch_indx, (data, d_idx), in enumerate(train_loader):
                        ...
                        self.optimizer.zero_grad()
                        recon_batch, x_low, qc, s, c, mu, log_var, log_qc = self.model(trans_data, temp[epoch], prior_c=prior_c, mask=pruning_mask)
                        loss, loss_rec, loss_joint, entropy, dist_c, d_qc, KLD_cont, min_var_0, loglikelihood = self.model.loss(recon_batch, trans_data, mu, log_var, qc, c, c_bin)
                        loss.backward()
                        self.optimizer.step()
                    ...
                    with torch.no_grad():
                        ...
                        for batch_indx, (data_val, d_idx), in enumerate(validation_loader):
                            ...
                            recon_batch, x_low, qc, s, c, mu, log_var, _ = self.model(x=trans_val_data, temp=self.temp, prior_c=prior_c, eval=True, mask=pruning_mask)
                            loss, loss_rec, loss_joint, _, _, _, _, _, _ = self.model.loss(recon_batch, trans_val_data, mu, log_var, qc, c, c_bin)
                            val_loss += loss.data.item()
                            for arm in range(self.n_arm):
                                val_loss_rec += loss_rec[arm].data.item()
        ...
        with torch.no_grad():
            for i, (data, d_idx) in enumerate(alldata_loader):
                ...
                recon, x_low, z_category, state, z_smp, mu, log_sigma, _ = self.model(trans_data, self.temp, prior_c=prior_c, eval=True, mask=pruning_mask)
                loss, loss_arms, loss_joint, _, dist_z, d_qz, _, _, loglikelihood = self.model.loss(recon, trans_data, mu, log_sigma, z_category, z_smp, c_bin)
    ...
    def eval_model(self, data_mat, c_p=[], c_onehot=[], batch_size=1000):
        ...
        with torch.no_grad():
            for i, (data, data_idx) in enumerate(data_loader):
                ...
                if self.ref_prior:
                    c_bin = torch.FloatTensor(c_onehot[data_idx, :])
                ...
                for arm in range(self.n_arm):
                    total_loss_rec[arm].append(loss_arms[arm].data.item() / self.input_dim)
                    total_loglikelihood[arm].append(loglikelihood[arm].data.item())
