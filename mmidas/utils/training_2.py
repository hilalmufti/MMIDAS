...
class train_cplmixVAE:
    ...
    def __init__(self, saving_folder='', n_feature=0, device=None, eps=1e-8, save_flag=True):
    ...
    def init_model(self, n_categories, state_dim, input_dim, fc_dim=100, lowD_dim=5, x_drop=0.2, s_drop=0.2, lr=.001,
                   ...
                   trained_model='', momentum=.01, n_zim=1):
        """
        ...
        self.model = cpl_mixVAE(input_dim=self.input_dim, fc_dim=fc_dim, n_categories=self.n_categories,
                                state_dim=self.state_dim,
                                lowD_dim=lowD_dim, x_drop=x_drop, s_drop=s_drop, n_arm=self.n_arm, lam=lam,
                                lam_pc=lam_pc,
                                tau=tau, beta=beta, hard=hard, variational=variational, device=self.device,
                                eps=self.eps,
                                ref_prior=ref_prior, momentum=momentum, n_zim=n_zim)
    ...
    def run(self, train_loader, test_loader, alldata_loader, n_epoch, n_epoch_p, g_idx, c_p=0, min_con=.5, max_pron_it=0, loss_mode='MSE'):
        """
        ...
        for epoch in range(n_epoch):
            ...
            for batch_indx, (data, d_idx), in enumerate(train_loader):
                ...
                for arm in range(self.n_arm):
                    g_ind0 = arm * self.input_dim
                    g_ind1 = (arm + 1) * self.input_dim
                    trans_data.append(data[:, g_idx[g_ind0:g_ind1]])
                ...
                loss, loss_rec, loss_joint, entropy, dist_c, d_qc, KLD_cont, min_var_0, loglikelihood = self.model.loss(
                    recon_batch, p_x, r_x, trans_data, mu, log_var, qc, c, prior_c, loss_mode)
                loss.backward()
                self.optimizer.step()
            ...
            print('====> Epoch:{}, Total Loss: {:.4f}, Loss_arms: {'':.4f}, Joint Loss: {:.4f}, '
                  'Entropy: {:.4f}, d_logqz: {:.4f}, d_qz: {:.4f}, var_min: {:.4f}, Elapsed Time:{:.2f}'.format(
                epoch, train_loss[epoch], np.mean(train_recon[:, epoch]), train_loss_joint[epoch],
                train_entropy[epoch], train_log_distance[epoch], train_distance[epoch], train_minVar[epoch],
                time.time() - t0))
            ...
            with torch.no_grad():
                ...
                for batch_indx, (data_val, d_idx), in enumerate(test_loader):
                    ...
                    for arm in range(self.n_arm):
                        g_ind0 = arm * self.input_dim
                        g_ind1 = (arm + 1) * self.input_dim
                        trans_val_data.append(data_val[:, g_idx[g_ind0:g_ind1]])
                    ...
                    recon_batch, p_x, r_x, x_low, qc, s, c, mu, log_var, _ = self.model(x=trans_val_data, temp=self.temp,
                                                                                   eval=True)
                    loss, loss_rec, loss_joint, _, _, _, _, _, _ = self.model.loss(recon_batch, p_x, r_x, trans_val_data, mu,
                                                                                   log_var, qc, c, prior_c, loss_mode)
            ...
            print('====> Validation Loss: {:.4f}'.format(validation_rec_loss[epoch]))
        ...
        if self.save and n_epoch > 0:
            ...
            ax.set_title(
                'Learning curve of the cpl-mixVAE for K=' + str(self.n_categories) + ' and S=' + str(self.state_dim))
            ax.spines['right'].set_visible(False)
            ax.spines['top'].set_visible(False)
            ax.legend()
            ax.figure.savefig(self.folder + '/model/learning_curve_before_pruning_K_' + str(
        ...
        with torch.no_grad():
            for i, (data, d_idx) in enumerate(train_loader):
                ...
                recon, p_x, r_x, x_low, z_category, state, z_smp, mu, log_sigma, _ = self.model(trans_data, self.temp, eval=True)
        ...
        for arm_a in range(self.n_arm):
            ...
            for arm_b in range(arm_a + 1, self.n_arm):
                ...
                armA_vs_armB = np.divide(armA_vs_armB, np.array(num_samp_arm), out=np.zeros_like(armA_vs_armB), where=np.array(num_samp_arm) != 0)
        ...
        while not stop_prune:
            ...
            with torch.no_grad():
                for i, (data, d_idx) in enumerate(train_loader):
                    ...
                    recon, p_x, r_x, x_low, z_category, state, z_smp, mu, log_sigma, _ = self.model(trans_data, self.temp,
                                                                                               mask=pruning_mask,
                                                                                               eval=True)

                    for arm in range(self.n_arm):
                        z_encoder = z_category[arm].cpu().data.view(z_category[arm].size()[0],
                                                                    self.n_categories).detach().numpy()
                        predicted_label[arm,
                        i * self.batch_size:min((i + 1) * self.batch_size, len(train_loader.dataset))] = np.argmax(
                            z_encoder, axis=1)
            ...
            for arm_a in range(self.n_arm):
                ...
                for arm_b in range(arm_a + 1, self.n_arm):
                    ...
                    plt.title('|c|=' + str(self.n_categories), fontsize=20)
                    plt.savefig(
                        self.folder + '/consensus_' + str(pr) + '_arm_' + str(arm_a) + '_arm_' + str(arm_b) + '.png',
                        dpi=600)
            ...
            if not stop_prune:
                ...
                for epoch in range(n_epoch_p):
                    ...
                    for batch_indx, (data, d_idx), in enumerate(train_loader):
                        ...
                        self.optimizer.zero_grad()
                        recon_batch, p_x, r_x, x_low, qz, s, z, mu, log_var, log_qz = self.model(trans_data, self.temp,
                                                                                            mask=pruning_mask)
                        loss, loss_rec, loss_joint, entropy, dist_z, d_qz, KLD_cont, min_var_0, _ = self.model.loss(
                            recon_batch, p_x, r_x, trans_data, mu, log_var, qz, z, prior_c, loss_mode)
                    ...
                    with torch.no_grad():
                        ...
                        for batch_indx, (data_val, d_idx), in enumerate(test_loader):
                            ...
                            loss, loss_rec, loss_joint, _, _, _, _, _, _ = self.model.loss(recon_batch, p_x, r_x,
                                                                                           trans_val_data,
                                                                                           mu, log_var, qc, c, prior_c, loss_mode)
                            val_loss += loss.data.item()
                            for arm in range(self.n_arm):
                                val_loss_rec += loss_rec[arm].data.item()
                ...
                trained_model = self.folder + '/model/cpl_mixVAE_model_after_pruning_' + str(
                    pr + 1) + '_' + self.current_time + '.pth'
                torch.save(
                    {'model_state_dict': self.model.state_dict(), 'optimizer_state_dict': self.optimizer.state_dict()},
                    trained_model)
                # plot the learning curve of the network
        ...
        with torch.no_grad():
            for i, (data, d_idx) in enumerate(alldata_loader):
                ...
                for arm in range(self.n_arm):
                    g_ind0 = arm * self.input_dim
                    g_ind1 = (arm + 1) * self.input_dim
                    trans_data.append(data[:, g_ind0:g_ind1])

                recon, p_x, r_x, x_low, z_category, state, z_smp, mu, log_sigma, _ = self.model(trans_data, self.temp,
                                                                                           eval=True, mask=pruning_mask)
                loss, loss_arms, loss_joint, _, dist_z, d_qz, _, _, loglikelihood = self.model.loss(recon, p_x, r_x,
                                                                                                    trans_data, mu,
                                                                                                    log_sigma,
                                                                                                    z_category, z_smp,
                                                                                                    prior_c, loss_mode)
                ...
                for arm in range(self.n_arm):
                    state_sample[arm, i * self.batch_size:min((i + 1) * self.batch_size, max_len), :] = state[
                        arm].cpu().detach().numpy()
                    state_mu[arm, i * self.batch_size:min((i + 1) * self.batch_size, max_len), :] = mu[
                        arm].cpu().detach().numpy()
                    state_var[arm, i * self.batch_size:min((i + 1) * self.batch_size, max_len), :] = log_sigma[
                        arm].cpu().detach().numpy()
                    z_encoder = z_category[arm].cpu().data.view(z_category[arm].size()[0],
                                                                self.n_categories).detach().numpy()
                    ...
                    x_low_all[arm, i * self.batch_size:min((i + 1) * self.batch_size, max_len), :] = x_low[
                        arm].cpu().detach().numpy()
                    ...
                    predicted_label[arm, i * self.batch_size:min((i + 1) * self.batch_size, max_len)] = np.array(
                        label_predict)
    ...
    def eval_model(self, data_loader, g_idx, c_p=[], c_onehot=[], batch_size=1000):
        ...
        with torch.no_grad():
            for i, (data, data_idx) in enumerate(data_loader):
                ...
                if self.ref_prior:
                    predicted_label[0, i * self.batch_size:min((i + 1) * self.batch_size, max_len)] = np.argmax(
                        c_p[data_idx, :], axis=1) + 1
                ...
                for arm in range(self.n_arm):
                    recon_cell[arm, i * self.batch_size:min((i + 1) * self.batch_size, max_len), :] = recon[
                        arm].cpu().detach().numpy()
                    state_sample[arm, i * self.batch_size:min((i + 1) * self.batch_size, max_len), :] = state[
                        arm].cpu().detach().numpy()
                    state_mu[arm, i * self.batch_size:min((i + 1) * self.batch_size, max_len), :] = mu[
                    ...
                    z_samp = z_smp[arm].cpu().data.view(z_smp[arm].size()[0], self.n_categories).detach().numpy()
                    z_sample[arm, i * self.batch_size:min((i + 1) * self.batch_size, max_len), :] = z_samp
                    data_low[arm, i * self.batch_size:min((i + 1) * self.batch_size, max_len), :] = x_low[
                        arm].detach().cpu().numpy()
                    ...
                    if self.ref_prior:
                        predicted_label[arm + 1,
                        i * self.batch_size:min((i + 1) * self.batch_size, max_len)] = np.argmax(z_encoder, axis=1) + 1
                    else:
                        predicted_label[arm, i * self.batch_size:min((i + 1) * self.batch_size, max_len)] = np.argmax(
                            z_encoder, axis=1) + 1
    ...
    def cluster_analysis(self, data_mat, ref_label, batch_size=1000):
        ...
        with torch.no_grad():
            for i, (data, labels) in enumerate(data_loader):
                ...
                recon, p_x, r_x, x_low, z_category, state, z_smp, mu, log_sigma, _ = self.model(trans_data, self.temp,
                                                                                           eval=True, mask=pruning_mask)
                _, _, _, entropy, dist_z, d_qz, _, min_var_0, _ = self.model.loss(recon, p_x, r_x, trans_data, mu, log_sigma,
                                                                                  z_category, z_smp, 0)
                # print(min_var_0)
