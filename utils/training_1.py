...
class train_cplmixVAE:
    ...
    def __init__(self, saving_folder='', aug_file='', n_feature=0, device=None, eps=1e-8, save_flag=True):
        ...
        if self.aug_file:
            ...
            self.netA = Augmenter_facs(noise_dim=self.aug_param['num_n'],
                                  latent_dim=self.aug_param['num_z'],
                                  input_dim=self.n_features).cuda(device)
    ...
    def run(self, train_loader, test_loader, validation_loader, alldata_loader, n_epoch, n_epoch_p, c_p=0, min_con=.5, max_pron_it=0):
        """
        ...
        for epoch in range(n_epoch):
            ...
            for batch_indx, (data, d_idx), in enumerate(train_loader):
                ...
                for arm in range(self.n_arm):
                    if self.aug_file:
                        noise = torch.randn(self.batch_size, self.aug_param['num_n'])
                        if self.gpu:
                ...
                recon_batch, x_low, qc, s, c, mu, log_var, log_qc = self.model(x=trans_data, temp=self.temp)
                loss, loss_rec, loss_joint, entropy, dist_c, d_qc, KLD_cont, min_var_0, loglikelihood = self.model.loss(recon_batch, trans_data, mu, log_var, qc, c, prior_c)
                loss.backward()
                self.optimizer.step()
            ...
            with torch.no_grad():
                ...
                for batch_indx, (data_val, d_idx), in enumerate(validation_loader):
                    ...
                    recon_batch, x_low, qc, s, c, mu, log_var, _ = self.model(x=trans_val_data, temp=self.temp, eval=True)
                    loss, loss_rec, loss_joint, _, _, _, _, _, _ = self.model.loss(recon_batch, trans_val_data, mu, log_var, qc, c, prior_c)
        ...
        while not stop_prune:
            ...
            with torch.no_grad():
                for i, (data, d_idx) in enumerate(alldata_loader):
                    ...
                    recon, x_low, z_category, state, z_smp, mu, log_sigma, _ = self.model(trans_data, self.temp, mask=pruning_mask, eval=True)
                    loss, loss_arms, loss_joint, _, _, _, _, _, _ = self.model.loss(recon, trans_data, mu, log_sigma, z_category, z_smp, prior_c)
                    ...
                    for arm in range(self.n_arm):
                        ...
                        predicted_label[arm, i * self.batch_size:min((i + 1) * self.batch_size, len(alldata_loader.dataset))] = np.array(label_predict)
            ...
            for arm_a in range(self.n_arm):
                ...
                for arm_b in range(arm_a+1, self.n_arm):
                    ...
                    plt.savefig(self.folder + '/consensus_' + str(pr) + '_arm_' + str(arm_a) + '_armE_' + str(arm_b) + '.png', dpi=600)
            ...
            if not stop_prune:
                ...
                for epoch in range(n_epoch_p):
                    ...
                    for batch_indx, (data, d_idx), in enumerate(train_loader):
                        ...
                        self.optimizer.zero_grad()
                        recon_batch, x_low, qz, s, z, mu, log_var, log_qz = self.model(trans_data, self.temp, mask=pruning_mask)
                        loss, loss_rec, loss_joint, entropy, dist_z, d_qz, KLD_cont, min_var_0, _ = self.model.loss(recon_batch, trans_data, mu, log_var, qz, z, batch_indx, prior_c)
                        loss.backward()
                        self.optimizer.step()
                    ...
                    with torch.no_grad():
                        ...
                        for batch_indx, (data_val, d_idx), in enumerate(validation_loader):
                            ...
                            recon_batch, x_low, qc, s, c, mu, log_var, _ = self.model(x=trans_val_data, temp=self.temp,
                                                                                      eval=True, mask=pruning_mask)
                            loss, loss_rec, loss_joint, _, _, _, _, _, _ = self.model.loss(recon_batch, trans_val_data,
                                                                                           mu, log_var, qc, c, prior_c)
                            val_loss += loss.data.item()
                            for arm in range(self.n_arm):
                                val_loss_rec += loss_rec[arm].data.item()
                ...
                fig, ax = plt.subplots()
                ax.plot(range(n_epoch), train_loss, label='Training')
                ax.plot(range(n_epoch), total_val_loss, label='Validation')
                ax.set_xlabel('# epoch', fontsize=16)
                ax.set_ylabel('loss value', fontsize=16)
        ...
        with torch.no_grad():
            for i, (data, d_idx) in enumerate(alldata_loader):
                ...
                recon, x_low, z_category, state, z_smp, mu, log_sigma, _ = self.model(trans_data, self.temp, eval=True, mask=pruning_mask)
                loss, loss_arms, loss_joint, _, dist_z, d_qz, _, _, loglikelihood = self.model.loss(recon, trans_data, mu, log_sigma, z_category, z_smp, prior_c)
    ...
    def eval_model(self, data_mat, c_p=[], batch_size=1000):
    ...
    def cluster_analysis(self, data_mat, ref_label, batch_size=1000):
        ...
        with torch.no_grad():
            for i, (data, labels) in enumerate(data_loader):
                ...
                recon, x_low, z_category, state, z_smp, mu, log_sigma, _ = self.model(trans_data, self.temp, eval=True, mask=pruning_mask)
                _, _, _, entropy, dist_z, d_qz, _, min_var_0, _ = self.model.loss(recon, trans_data, mu, log_sigma, z_category, z_smp, 0)
                # print(min_var_0)
